...
本地验收：`pnpm run smoke:channels`（API 须已在 :13008；P0 校验 OpenAPI 含 `POST /payments/receipts` 与 receipt `pending` / `{receiptId}` get / `apply-ledger` / `apply-ledger-batch`，以及 `POST /payments/ledger/snapshot` 与 `GET /payments/ledger/snapshots/latest`）。
