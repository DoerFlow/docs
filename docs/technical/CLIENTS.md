...
- **web `/ecosystem`**：只读 catalog / jobs；档位 Tag 来自 `/capabilities`。部署档位低于 `agent-commerce` 时展示 `ecosystem.labHint` Alert；catalog 的 readiness Tag 区分 `lab` 与 `production`。未登录或无租户为空态。**不是**生产合作方控制台。见 [ECOSYSTEM.md](./ECOSYSTEM.md)。
