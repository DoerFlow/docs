...
`smoke:m5` 检查：生产文档与 env 模板（含 `COMMERCIAL_MODE`）、Hardhat `base`、探针与 disclosure（含 `/ready` 状态码与 `/version` 的 `profile`/`manifestHash`）、OpenAPI 源文件收据重试路径字符串（含 `apply-ledger-batch`）、M4 SDK、compose 基座与 overlay、`use:base`。  
